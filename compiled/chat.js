(function() {
  ...
  window.Chat = Chat = (function() {
    ...
    Chat.prototype.socketOpen = function() {
      var name;
      if ($("#contact_list").size() === 0) $("body").append(this.dom_elem);
      name = window.location.hash.substr(1);
      if (name !== "") {
        this.name = name;
      } else {
        this.name = prompt("What's your name ?");
        if (this.name) window.location.hash = this.name;
      }
      if (this.name) {
    ...
    Chat.prototype.socketMessage = function(message) {
      ...
      switch (json.type) {
        ...
        case 'event':
          ...
          if (conversation) return conversation.addEvent(json.data);
          break;
        case "list":
          this.name = json.data.from;
          $(".chat-list", this.dom_elem).html('');
          _ref = json.data.val;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            c = _ref[_i];
            $(".chat-list", this.dom_elem).append("<div class='contact'>" + c + "</div>");
          }
          _ref2 = this.conversations;
          _results = [];
          for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
            conversation = _ref2[_j];
            if (_ref3 = conversation.receiver, __indexOf.call(json.data.val, _ref3) >= 0) {
              _results.push(conversation.enable());
            } else {
              _results.push(conversation.disable());
            }
          }
          return _results;
      }
    ...
    Chat.prototype.getConversation = function(sender) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (conversation.receiver === sender) return conversation;
      }
    };
    ...
    Chat.prototype.checkConnexion = function() {
      if (this.socket && this.socket.readyState !== 1) {
        ...
        if (!this.socket || !this.name) this.initializeSocket();
        if (this.connected()) return this.enable();
      }
    };
